...
class App extends React.Component {
  ...
  myaxiosfuncSum=e=>{
    ...
    axios.get("http://localhost:8000/"+this.state.present+"/"+e.target.className).then(e=>{this.setState({data:e.data})}).catch(err=>alert(err))
  }
  myaxiosfunc=e=>{
    var l=[]
    for (var y=1;y<=6;y++){
    ...
    axios.get("http://localhost:8000/"+e.target.className).then(e=>{this.setState({data:e.data});
  ...
  componentDidMount(){
    axios.get("http://localhost:8000/3").then(e=>{this.setState({data:e.data})}).catch(err=>alert(err))
  }
